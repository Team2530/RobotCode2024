...
public class LimeLightSubsystem extends SubsystemBase {
    ...
    public void periodic() {
        try {
            ...
            // If we see april tag(s), we update the dictionary if needed, with position,
            // capture time, and tag ID
            if (results.targetingResults.targets_Fiducials.length > 0) {
                ...
                // Loops through dictionary of april tags, and if it's been over a second from
                // capture time, removes the tag
                ...
                if (LimelightConstants.LOG_APRIL_TAGS_INTO_SMARTDASH_BOARD) {
                    ...
                    // Loops through values of dictionary, for each april tag, prints ID as well as
    ...
    public KnownAprilTag getKnownAprilTag(boolean isItForRightSideAprilTag) {
        ...
        if (alliance.isPresent()) {
            ...
            for (String s : tagIdsForThisAction) {
                aprilTag = lastKnownAprilTags.get(s);
                SmartDashboard.putString("I am inside 1 : ", s);
                if (aprilTag != null) {
